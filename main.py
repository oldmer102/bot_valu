...
def help_message(message):
    BOT.send_message(message.chat.id, f"Давай помогу с примерами как я работаю. Для начала необходимо запросить список валют, которые мне предоставляет ЦБ РФ.  Если мы ходим конвертировать валюту в рубли необходимо написать короткое название валюты и сумму, которую хотим узнать: EUR 20")

@BOT.message_handler(commands=['values'])
def values_help_message(message):
    BOT.send_message(message.chat.id, "Список всех валют которые я знаю: \n" + MSG + 'RUB: Российский рубль')
...
def test_message(message):
    ...
    if not price.conversion_rule():
        BOT.send_message(message.chat.id,'Я не знаю валюту: ' + price.conversion_rule())
